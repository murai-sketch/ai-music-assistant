...
選び方の考え:
  ...
  - サビ（Chorus / Hook）の頭から始まる区間を最優先する
    （ショートは最初の数秒で離脱が決まるため、頭出しがサビ以外だと弱い）
...
def find_shorts(alignment, sections=None, beats=None, duration=None,
    ...
    for i in range(len(spans)):
        ...
        for j in range(i, len(spans)):
            ...
            reasons = []
            score = KIND_WEIGHT.get(kinds[i], 1.0)
            ...
            cands.append({
                ...
                "label": f"{name}から{j - i + 1}行"
                         + ("・サビ入り" if covered and kinds[i] != "hook" else ""),
